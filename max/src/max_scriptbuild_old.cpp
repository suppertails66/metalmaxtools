...
try {
  ...
  if (argc < 4) {
    ...
    cout << "Usage: " << argv[0] << "<inrom> <outrom> <buildscript>" << endl;
...
//  TIniFile buildscript = TIniFile(string(argv[1]));
  TIniFile buildscript = TIniFile(string(argv[3]));
...
//  std::string inromName = buildscript.valueOfKey("Properties", "InputRom");
  ...
  // copy final 2 prg banks to new end pos
/*  {
    char buffer[inputPrgFinalBanksSize];
    prgofs.seek(inputPrgFinalBanksPos);
    prgofs.read(buffer, inputPrgFinalBanksSize);
    prgofs.seek(outputPrgFinalBanksPos);
    prgofs.write(buffer, inputPrgFinalBanksSize);
  } */
...
//  std::string outromName = buildscript.valueOfKey("Properties", "OutputRom");
